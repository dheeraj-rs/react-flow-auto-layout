...
function TestFlows() {
  ...
  const createEdge = (nodesArray, data) => {
    ...
    if (show?.prevNodeId === 'incoming-call') {
      edge = {
        ...
        targetHandle: `${data?.type}:target:${
          nodesArray[nodesArray.length - 1]?.id
        }`,
      ...
      edges.length > 0 &&
      edges[edges.length - 1]?.id === 'incoming-call'
    ) {
      edge = {
        id: `edge-${id}`,
        source: show?.prevNodeId,
  ...
  const handleAddComponent = (data) => {
    ...
    if (data.prevNodeId === 'incomingCall') {
      handleDeleteNode('node-2', setNodes).then((response) => {
        const nodesArray = handleUpdateLayout(
          response,
          newNode,
          setNodes,
          setFlowNodes,
        ...
        const newNodes = customLayout(nodesArray, edges);
        setNodes(newNodes);
        setFlowNodes(newNodes);
      });
    } else {
  ...
  const handleDeleteComponent = () => {
    ...
    handleDeleteNode(selectedNodeId, setNodes).then((updatedNodesArray) => {
      handleRemoveConnection(edgeAsTarget?.id, edges).then((result) => {
        const updatedEdgesArray = result.filter(
          (e) => !edgeAsSource.includes(e)
        );
  ...
  const onEdgeClick = (event, e) => {
    ...
    if (result.length > 0) {
      setActiveNodesDetails({
        sourceId: result[0]?.source,
        targetId: result[0]?.target,
      });
    } else {
      setActiveNodesDetails({ sourceId: '', targetId: '' });
    }
  ...
  useEffect(() => {
    if (callFlowDetails?.data?.attributes?.flow) {
      const { flow } = callFlowDetails.data.attributes;
      if (flow.actions.length > 0 && flow.connections.length > 0) {
        const newNodes = customLayout(flow.actions, flow.connections);
        setNodes(newNodes);
        setFlowNodes(newNodes);
              ...
              <div className="row">
                <div className="call-header-outer">
                  <Subhead
                    ...
                    name={callFlowDetails?.data?.attributes?.name}
                    isPublished={
                      callFlowDetails?.data?.attributes?.is_published
                    }
